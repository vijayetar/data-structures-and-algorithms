...
const forEachTwoToThe = (arr) => {
  let newArray = [];
  arr.forEach(el => newArray.push(Math.pow(2,el))); 
  return newArray;
}
...
const charCode = (arr) => {
  let newArray = [];
  const chartCode = arr.map(strel => strel.charCodeAt());
  newArray.push(chartCode);
  return newArray;
}
...
const extractAbilities = (arr) => {
  const abilityArray = [];
  arr.map(el => {
    abilityArray.push(el.ability.name);
  })
  return abilityArray;
};
...
const extractStats = (arr) => {
  // use map
  // return array of objects with stat name and total
  // total is sum of effort and baseStat
  arr.map(el => {
  const statname = el.stat.name;
  const total = el.effort + el.baseStat;
  new Stat(statname, total);
})
};
const newArray = [];
function Stat(name,total){
  this.name = name;
  this.total = total;
  newArray.push(this);
}
