...
const validateEmail = (email) => {
  let regEx = /^[a-z0-9]?\w+(.\w+)@{1}\w+[(.com)(.org)(.net)]{4}\b/;
...
function findTagNames(arr) {
  let regEx = /(\/)\w+/;
  const newArray = [];
  arr.forEach(str => {
  console.log('matched strings', str.match(regEx))});
  const newstring = arr.reduce((accl, str, idx) => {
    accl = str.match(regEx) + accl;
    newArray.push(accl);
    return accl;
  }, '');
  const anotherArray = [];
  arr.forEach((str) => anotherArray.push(str.match(regEx)));
  for (var i =0; i<anotherArray.length; i++) {
    for (var j=0; j<anotherArray[i].length; j++) {
      return anotherArray[i][j];
    }
  }
};
