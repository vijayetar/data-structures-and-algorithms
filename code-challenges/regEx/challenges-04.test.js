...
const isCapitalized = (str) => {
  ...
  if (str.match(validator)=== null){ return arr} else {return str.match(validator)};
};
...
const citiesAtoJ = (arr) => {
  const regex = /\S\b[A-J][a-z]+\S/g;
  arr.forEach(cityName => {
    console.log(cityName);
    if (cityName === arr.match(regex)){newArr.push(cityName)};
  })
  return newArr;
};
